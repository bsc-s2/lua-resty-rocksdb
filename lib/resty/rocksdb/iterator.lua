...
function _M.new(db, readoptions)
    ...
    if iter == nil then
        return nil, 'CreateIterError', nil
    end
...
function _M.seek_to_first(self)
    return rocksdb.rocksdb_iter_seek_to_first(self.iter)
end

function _M.seek_to_last(self)
    return rocksdb.rocksdb_iter_seek_to_last(self.iter)
end

function _M.seek(self, key)
    return rocksdb.rocksdb_iter_seek(self.iter, key, #key)
end
...
function _M.next(self)
    return rocksdb.rocksdb_iter_next(self.iter)
end

function _M.prev(self)
    return rocksdb.rocksdb_iter_prev(self.iter)
end
...
function _M.destroy(self)
    return rocksdb.rocksdb_iter_destroy(self.iter)
end
